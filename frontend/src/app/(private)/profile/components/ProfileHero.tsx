...
import { Calendar, Mail, MapPin, User, Settings2 } from "lucide-react";
...
export function ProfileHero({
  ...
  // Calculate level progress percentage (assuming 1000 XP per level for now or based on some logic)
  // If rank/level logic is more complex, we might need a helper
  ...
  const progressPercent = Math.min(100, Math.max(0, (xpInCurrentLevel / (nextLevelXp - currentLevelXp)) * 100));
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 flex-wrap mb-1">
            ...
            <Button
              ...
              className="h-7 px-3 border-primary/30 bg-primary/5 hover:bg-primary/10 text-primary font-mono text-[10px] uppercase tracking-widest gap-1.5"
            >
              <Settings2 className="h-3 w-3" /> {labels.editProfile}
            </Button>
          </div>
